...
import LeveragedTokenHeader from "../../assets/img/levraged-token-header.png";
      ...
      <div className="px-4 sm:px-6 text-right max-w-[1400px] mx-auto my-10">
        <div className="grid grid-cols-9 items-center my-10 space-y-5 bg-gray-50 dark:bg-[#051a36] rounded-3xl p-16 rtl-grid">
          <div className="flex flex-col space-y-5 lg:col-span-3  col-span-9">
            ...
            <p className="leading-relaxed">
              {leveraged_tokens_data.header.text}
            </p>
            <Link className="flex" to={"/redirect-to-platform"}>
              <button className="text-[#f39200] cursor-pointer transition-all px-8 py-4 bg-[#f39200] bg-opacity-10 rounded-md">
                {leveraged_tokens_data.header.buttonText}
              </button>
            </Link>
          </div>
          <div className="lg:col-span-6 flex col-span-9">
            <img
              src={LeveragedTokenHeader}
        ...
        <div className="grid grid-cols-8 rtl-grid gap-8">
          <div className="lg:col-span-5 md:col-span-8 col-span-8">
            <div className="xl:h-[450px] lg:h-[500px] md:h-[400px] sm:h-[400px] max-sm:h-[200px] w-full loading-animation rounded-3xl">
              <iframe
                src={
          ...
          <div className="lg:col-span-3 md:col-span-8 col-span-8 flex flex-col space-y-10 leading-relaxed bg-gray-50 dark:bg-[#051a36] rounded-3xl p-10 text-justify text-[14px]">
            <p>{leveraged_tokens_data.info.text_1}</p>
            <p>{leveraged_tokens_data.info.text_2}</p>
          </div>
        </div>
        <div className="my-10 flex flex-col justify-center items-center bg-gray-50 dark:bg-[#051a36] rounded-3xl p-16 rtl-grid">
          <h2 className="text-[#f39200] text-3xl font-semibold">
            {leveraged_tokens_data.symbols.title}
          </h2>
          <p className="mt-3 w-3/4 text-center">
            {leveraged_tokens_data.symbols.text}
          </p>
          <div className="w-full grid lg:grid-cols-2 grid-cols-1 gap-5">
            {leveraged_tokens_data.symbols.pictures.map((item) => {
              return (
                <div
                  key={item.id}
